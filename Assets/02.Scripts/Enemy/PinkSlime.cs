...
public class PinkSlime : MonoBehaviour
{
    ...
    private AudioSource pinkAudio;  // 핑크슬라임 오디오 소스
    public AudioClip pinkFollow;    // 공격 오디오
    ...
    void Start()
    {
        pinkAudio = GetComponent<AudioSource>(); // AudioSource Component 가져오기
    ...
    void Update()
    {
        ...
        {
            if (!pinkAudio.isPlaying)              // 오디오가 실행되지 않고 있으면
            {
                pinkAudio.PlayOneShot(pinkFollow); // 공격 오디오 플레이
            }
            FollowPlayer();
        }
