...
public class YellowSlime : MonoBehaviour
{
    ...
    private AudioSource yellowlAudio; // 옐로우 슬라임 오디오 소스
    public AudioClip yellowFollow;    // 공격 오디오
    ...
    void Start()
    {
        yellowlAudio = GetComponent<AudioSource>(); // AudioSource Component 가져오기
    ...
    void Update()
    {
        ...
        {
            if (!yellowlAudio.isPlaying)               // 오디오가 실행되지 않고 있으면
            {
                yellowlAudio.PlayOneShot(yellowFollow); // 공격 오디오 플레이
            }
            FollowPlayer();
        }
    ...
    public void GoHome()
    {
        ...
        {
            yellowlAudio.Stop(); // 공격을 멈추면 소리 멈춤
